...
:::tip
文字コードと改行コードは [共通ルール](./common.md) を参照。
:::
...
- 禁止事項の検出。IDセレクタ、深いネスト、タグとクラスの併用
- 命名規則の検証。BEM準拠
- プロパティの記述順序の統一
...
### 2.3 Defensive なCSS設計

Webサイトは継続的に更新と改修が発生する。
...
すべてのプロパティに意味を持たせる。各プロパティは必要性を説明できる状態で指定する。
...
`font` `background` `flex` などのショートハンドは未指定値までリセットされ、副作用が出やすい。
意図を理解した上で使用する。部分的な変更は個別プロパティを使う。
...
- 一般的で誤解のない略語は許容する（例: `.navigation` → `.nav`）
- 難解な単語は避ける（例: `auxiliary` より `support`）
- 日本独自概念はローマ字を許容する（例: `omikuji` `torii`）
